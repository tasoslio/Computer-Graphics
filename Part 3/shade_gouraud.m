function Y = shade_gouraud(vertsp, vertsn, vertsc, bcoords, cam_pos, mat, lights, Ia, X)
    ...
    %% shade_gouraud is a function that paints with the gouraud method a triangle
    % VARIABLES
    ...
    % Find the Iam, Idif, Isp
    for k = 1 : 3
        Iam = ambient_light(mat, vertsc(:, k), Ia');
        Idif = diffuse_light(bcoords, vertsn(:, k), Iam, mat, lights);
        Isp = specular_light(bcoords, vertsn(:, k), Idif, cam_pos, mat, lights);
        vertex_colors(k, :) = Isp;
    end
    ...
    if ver_same_line == 1 
        ...
        for y = min_y : max_y
            ...
            X(y, vertices_2d(1,1), :) = paint_color;
    ...
    elseif hor_same_line == 1
        ...
        for x = min_x : max_x
            ...
            X(vertices_2d(1,2), x, :) = paint_color;
